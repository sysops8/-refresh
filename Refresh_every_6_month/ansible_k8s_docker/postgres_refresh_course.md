...
0 * * * * /path/to/postgres_maintenance.sh check

# Обслуживание каждую ночь в 2:00
0 2 * * * /path/to/postgres_maintenance.sh maintain

# Еженедельный отчет (воскресенье в 8:00)
0 8 * * 0 /path/to/postgres_maintenance.sh report
```

### Задание:

1. **Создай схему мониторинга:**
   - Создай все таблицы для хранения метрик
   - Создай все функции и процедуры
   - Создай представления для отчетов

2. **Протестируй сбор метрик:**
   - Запусти `collect_db_metrics()`
   - Запусти `detect_slow_queries()`
   - Запусти `check_table_bloat()`
   - Проверь, что данные записались в таблицы

3. **Проверь здоровье БД:**
   - Выполни `SELECT * FROM db_health_dashboard;`
   - Проверь топ медленных запросов
   - Найди проблемные таблицы

4. **Запусти обслуживание:**
   - Выполни `CALL auto_vacuum_bloated_tables();`
   - Выполни `CALL cleanup_old_metrics(30);`

5. **Автоматизация:**
   - Создай bash скрипт
   - Настрой права на выполнение
   - Добавь в cron

6. **Документация:**
   - Создай README с описанием системы
   - Опиши как использовать каждую функцию
   - Добавь примеры использования

### Критерии оценки:

- ✅ Все функции и процедуры созданы и работают
- ✅ Метрики собираются корректно
- ✅ Представления выводят актуальные данные
- ✅ Процедуры обслуживания работают без ошибок
- ✅ Bash скрипт автоматизирует все задачи
- ✅ Логирование настроено
- ✅ Документация понятна и полная

### Дополнительные улучшения:

- Добавь уведомления (email, Slack, Telegram)
- Интегрируй с Prometheus/Grafana
- Добавь проверку репликации
- Создай дашборд с историческими данными
- Настрой автоматическое создание бэкапов при критических событиях

---

## Справочная секция: Быстрые шпаргалки

### Горячие клавиши psql
...
\g                    # Выполнить последнюю команду
\p                    # Показать буфер команд
\r                    # Очистить буфер
\w filename           # Сохранить буфер в файл
```

### Полезные алиасы

Добавь в `~/.bashrc` или `~/.psqlrc`:
...
# .psqlrc (для psql)
\set PROMPT1 '%[%033[1;33m%]%n@%/%[%033[0m%]%# '
\set PROMPT2 '%[%033[1;33m%]%n@%/%[%033[0m%]%# '
\pset null '(null)'
\timing on
\x auto
\set HISTSIZE 10000
...
-- Полезные команды
\set activity 'SELECT pid, usename, application_name, client_addr, state, query FROM pg_stat_activity WHERE state != \'idle\';'
\set bloat 'SELECT schemaname, tablename, n_dead_tup, n_live_tup FROM pg_stat_user_tables WHERE n_dead_tup > 0 ORDER BY n_dead_tup DESC LIMIT 10;'
...
\set size 'SELECT schemaname, tablename, pg_size_pretty(pg_total_relation_size(schemaname||\'.\'||tablename)) as size FROM pg_tables ORDER BY pg_total_relation_size(schemaname||\'.\'||tablename) DESC LIMIT 10;'
...
# Долгие запросы
psql -U postgres -c "SELECT pid, now() - query_start as duration, query FROM pg_stat_activity WHERE state != 'idle' AND query_start < now() - interval '1 minute' ORDER BY duration DESC;"

# Топ-10 самых больших таблиц
psql -U postgres -d mydb -c "SELECT schemaname||'.'||tablename, pg_size_pretty(pg_total_relation_size(schemaname||'.'||tablename)) FROM pg_tables ORDER BY pg_total_relation_size(schemaname||'.'||tablename) DESC LIMIT 10;"

# Проблемы с блокировками
psql -U postgres -c "SELECT pid, usename, pg_blocking_pids(pid) as blocked_by, query FROM pg_stat_activity WHERE cardinality(pg_blocking_pids(pid)) > 0;"
```
...
```sql
-- Самые долгие запросы (требует pg_stat_statements)
SELECT 
...
-- Отсутствующие индексы (таблицы с seq_scan)
SELECT 
    schemaname,
    tablename,
    seq_scan,
...
**Проблемы с памятью:**
```sql
-- Статистика temp файлов (если work_mem мало)
SELECT 
    datname,
    temp_files,
...
**Для небольшого продакшна (Small Production):**
```ini
...
**Для большого продакшна (Large Production):**
```ini
...
-- Таблицы без владельца или с необычными правами
SELECT 
    schemaname,
    tablename,
    tableowner,
...
-- btree_gin, btree_gist (индексы для композитных типов)
...
- Пройди модули 1-6 обязательно (основы)
...
### Для закрепления:
- Создай мониторинг для своих БД
- Автоматизируй бэкапы и обслуживание
- Оптимизируй медленные запросы в проектах
- Настрой репликацию (если требуется)
- Практикуй troubleshooting на реальных проблемах

### Что отслеживать при повторных прохождениях:
- ✅ Помню ли основные команды и концепции?
- ✅ Могу ли быстро диагностировать проблемы производительности?
- ✅ Уверенно ли работаю с бэкапами и восстановлением?
- ✅ Знаю ли как настроить безопасность и мониторинг?
- ✅ Могу ли оптимизировать запросы и индексы?

---

## Советы по работе с PostgreSQL

1. **Используй EXPLAIN ANALYZE** перед оптимизацией запросов. Не гадай - измеряй.

2. **Регулярно делай VACUUM ANALYZE**. Автовакуум не всегда справляется с высокой нагрузкой.

3. **Мониторь cache hit ratio**. Если меньше 95% - увеличь shared_buffers.

4. **Не бойся индексов, но и не создавай их слепо**. Каждый индекс замедляет INSERT/UPDATE.

5. **Используй партиционирование** для больших таблиц (> 10GB).

6. **Настрой логирование медленных запросов** (log_min_duration_statement).

7. **Делай регулярные бэкапы** и ПРОВЕРЯЙ их восстановление.

8. **Используй connection pooling** (PgBouncer) для высоконагруженных приложений.

9. **Читай документацию PostgreSQL** - она отличная: https://www.postgresql.org/docs/

10. **Тестируй изменения на копии production** перед применением.

---

## Чеклист навыков PostgreSQL DevOps/SysAdmin

После прохождения курса ты должен уметь:

### Базовые навыки:
- [ ] Подключаться к PostgreSQL разными способами
- [ ] Создавать и управлять базами данных, таблицами, индексами
- [ ] Писать эффективные SQL запросы
- [ ] Понимать и использовать JOIN, подзапросы, CTE

### Производительность:
- [ ] Использовать EXPLAIN/EXPLAIN ANALYZE
- [ ] Создавать правильные индексы
- [ ] Оптимизировать медленные запросы
- [ ] Мониторить cache hit ratio и другие метрики
- [ ] Находить и устранять bloat

### Транзакции и блокировки:
- [ ] Работать с транзакциями и уровнями изоляции
- [ ] Понимать механизмы блокировок
- [ ] Диагностировать deadlock и blocking
- [ ] Использовать FOR UPDATE/FOR SHARE правильно

### Бэкап и восстановление:
- [ ] Делать логические бэкапы (pg_dump/pg_restore)
- [ ] Настраивать WAL архивирование
- [ ] Выполнять Point-in-Time Recovery
- [ ] Настраивать репликацию
- [ ] Автоматизировать бэкапы

### Безопасность:
- [ ] Управлять пользователями и ролями
- [ ] Настраивать права доступа (GRANT/REVOKE)
- [ ] Использовать Row Level Security
- [ ] Настраивать аутентификацию (pg_hba.conf)
- [ ] Включать SSL/TLS шифрование
- [ ] Настраивать аудит и логирование

### Мониторинг:
- [ ] Использовать системные представления (pg_stat_*)
- [ ] Настраивать pg_stat_statements
- [ ] Мониторить соединения и блокировки
- [ ] Анализировать логи
- [ ] Создавать дашборды мониторинга

### Администрирование:
- [ ] Управлять службой PostgreSQL
- [ ] Настраивать postgresql.conf
- [ ] Выполнять VACUUM, ANALYZE, REINDEX
- [ ] Управлять расширениями
- [ ] Обновлять PostgreSQL

### Программирование:
- [ ] Создавать функции и процедуры (PL/pgSQL)
- [ ] Работать с триггерами
- [ ] Использовать window functions
- [ ] Создавать представления и материализованные представления
...
**Мониторинг:**
- pg_stat_statements: Встроенное расширение
- pgwatch2: Мониторинг PostgreSQL
- postgres_exporter: Экспорт метрик в Prometheus
- pgmetrics: Snapshot метрик

**Практика:**
- Подними свой PostgreSQL сервер (локально или в облаке)
- Мигрируй проекты с MySQL/SQLite на PostgreSQL
- Участвуй в PostgreSQL сообществе
- Читай и анализируй query plans
...
**PostgreSQL это:**
- Один из самых продвинутых open-source RDBMS
- Соответствует стандартам SQL
- Расширяемый и гибкий
- Отличная документация и сообщество

**Помни:**
- Производительность начинается с правильной схемы данных
- Индексы решают 90% проблем производительности
- Мониторинг предотвращает большинство инцидентов
- Бэкапы бесполезны, если их нельзя восстановить
- Безопасность - это не опция, а требование

**Проходи этот курс каждые 6-12 месяцев, и PostgreSQL станет твоим надежным инструментом!** 🐘🚀

*"In PostgreSQL we trust, but always verify with EXPLAIN ANALYZE!"*верка статуса
